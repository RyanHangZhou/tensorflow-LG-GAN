...
import point_resnet as res_model
...
def evaluate(num_votes=1):
    ...
    with tf.device('/gpu:'+str(0)):
        ...
        with tf.variable_scope('foo'):
            ...
            generator_pc, _ = lgnet.get_gen_model(data_pl, labels_onehot, is_training_pl_ae, scope='generator', bradius=1.0,
                                                    reuse=None, use_normal=False, use_bn=False, use_ibn=False,
                                                    bn_decay=bn_decay, up_ratio=1)

        d_real = res_model.get_discriminator(data_pl, True, 'd_1', reuse=False, use_bn=False, use_ibn=False,
                                            use_normal=False, bn_decay=bn_decay)
        d_fake = res_model.get_discriminator(generator_pc, True, 'd_1', reuse=True, use_bn=False, use_ibn=False,
                                            use_normal=False, bn_decay=bn_decay)
        ...
        for var in tf.global_variables():
            if var.name.startswith('foo'):
                g_var.append(var)
            elif var.name.startswith('d_'):
                d_var.append(var)
            else:
                net_var.append(var)

        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            d_optim = tf.train.AdamOptimizer(d_lr, beta1=0.5).minimize(d_loss, var_list=d_var, colocate_gradients_with_ops=True)
            g_optim = tf.train.AdamOptimizer(g_lr, beta1=0.9).minimize(g_loss, var_list=g_var, colocate_gradients_with_ops=True)
        clip_D = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in d_var]
        ...
        for test_fn in range(len(TEST_FILES)):
            ...
            for test_batch_idx in range(test_num_batches):
                ...
                for test_vote_idx in range(num_votes):
                    ...
                    _, score, test_adv_data = sess.run([g_optim, pred, generator_pc], feed_dict=feed_dict)
                    _, score, test_adv_data = sess.run([d_optim, pred, generator_pc], feed_dict=feed_dict)
            ...
            for batch_idx in range(num_batches):
                ...
                for vote_idx in range(num_votes):
                    ...
                    feed_dict = {data_pl: rotated_data,
                             ...
                             is_training_pc: is_training,
                             is_training_pl_ae: is_training_ae}
                    _ = sess.run([g_optim], feed_dict=feed_dict)
                    _ = sess.run([d_optim], feed_dict=feed_dict)
