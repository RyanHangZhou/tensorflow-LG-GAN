...
from utils.pointnet_util import pointnet_sa_module,pointnet_fp_module, pointnet_sa_module_bn
import numpy as np
...
def get_gen_model(point_cloud, labels_onehot, is_training, scope, bradius = 1.0, reuse=None, use_rv=False, use_bn = False,use_ibn = False,
    ...
    with tf.variable_scope(scope,reuse=reuse) as sc:
        ...
        with tf.variable_scope('up_layer',reuse=reuse):
            ...
            labels_onehot = tf.expand_dims(labels_onehot, 1)
            labels_onehot = tf.expand_dims(labels_onehot, 1)
            labels_onehot = tf.tile(labels_onehot, [1, num_point, 1, 1])
            net = tf.concat([net, labels_onehot], 3)
...
def get_gen_model_m(point_cloud, labels_onehot, is_training, scope, bradius = 1.0, reuse=None, use_rv=False, use_bn = False,use_ibn = False,
                  use_normal=False,bn_decay=None, up_ratio = 4,idx=None):
    ...
    with tf.variable_scope(scope,reuse=reuse) as sc:
        ...
        labels_onehot1 = tf.tile(tf.expand_dims(labels_onehot, 1), [1, num_point, 1])
        up_l2_points = tf.concat([up_l2_points, labels_onehot1], 2)

        labels_onehot1 = tf.tile(tf.expand_dims(labels_onehot, 1), [1, num_point, 1])
        up_l3_points = tf.concat([up_l3_points, labels_onehot1], 2)

        labels_onehot1 = tf.tile(tf.expand_dims(labels_onehot, 1), [1, num_point, 1])
        up_l4_points = tf.concat([up_l4_points, labels_onehot1], 2)
        ...
        with tf.variable_scope('up_layer',reuse=reuse):
            ...
            for i in range(up_ratio):
                ...
                concat_feat = tf_util2.conv2d(concat_feat, 256, [1, 1],
                                              ...
                                              bn=False, is_training=is_training,
                                              scope='fc_layer0_%d'%(i), bn_decay=bn_decay)
                labels_onehot1 = tf.tile(tf.expand_dims(tf.expand_dims(labels_onehot, 1), 1), [1, num_point, 1, 1])
                concat_feat = tf.concat([concat_feat, labels_onehot1], 3)
            ...
            net = tf.concat(new_points_list, axis=1)
            labels_onehot1 = tf.tile(tf.expand_dims(tf.expand_dims(labels_onehot, 1), 1), [1, num_point, 1, 1])
            net = tf.concat([net, labels_onehot1], 3)
...
def get_gen_model2(point_cloud, labels_onehot, is_training, scope, bradius = 1.0, reuse=None, use_rv=False, use_bn = False,use_ibn = False,
                  use_normal=False,bn_decay=None, up_ratio = 4,idx=None):

    with tf.variable_scope(scope,reuse=reuse) as sc:
        labels_onehot = tf.cast(labels_onehot, tf.float32)
        # labels_onehot = tf.tile(labels_onehot, [4, 1])
        batch_size = point_cloud.get_shape()[0].value
        num_point = point_cloud.get_shape()[1].value
        l0_xyz = point_cloud[:,:,0:3]
        if use_normal:
        ...
        l2_xyz, l2_points, l2_indices = pointnet_sa_module_bn(labels_onehot[:, 32:40], l1_xyz, l1_points, npoint=num_point/2, radius=bradius*0.1,bn=True,ibn = use_ibn,
                                                           nsample=32, mlp=[64, 64, 128], mlp2=None, group_all=False,
                                                           is_training=is_training, bn_decay=bn_decay, scope='layer2')
        ...
        l4_xyz, l4_points, l4_indices = pointnet_sa_module_bn(labels_onehot[:, 0:32], l3_xyz, l3_points, npoint=num_point/8, radius=bradius*0.3,bn=True,ibn = use_ibn,
                                                           nsample=32, mlp=[256, 256, 512], mlp2=None, group_all=False,
                                                           is_training=is_training, bn_decay=bn_decay, scope='layer4')
